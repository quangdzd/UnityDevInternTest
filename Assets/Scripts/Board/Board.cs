...
public class Board
{
    ...
    internal void ReceiveRedo(Vector2Int index , Cell cell )
    {
        Cell c = m_cells[index.x , index.y];
        if(c.Item == null)
        {
            Item item = cell.Item;
            cell.Free();
            ...
            item.View.DOMove(c.transform.position, 0.3f);
        }
        Debug.Log("SUCCESS");
    }
    ...
    internal Cell FindFirstNonEmptyCell()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
    ...
    internal Cell FindCellWithNormalTypeExcept(List<NormalItem.eNormalType> eNormalTypes)
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                if(m_cells[x,y].Item != null)
                {
                    ...
                    if(!eNormalTypes.Contains(normalItem.ItemType))
                    {
                        return m_cells[x,y];
                    }
                }
    ...
    internal Cell FindCellToLose(List<Cell> cells)
    {
        Dictionary<NormalItem.eNormalType , int> kvpItemType = new Dictionary<NormalItem.eNormalType, int>();
        ...
        foreach(Cell cell in cells)
        {
            if(cell.Item != null)
            {
                ...
                if (!kvpItemType.ContainsKey(normalItem.ItemType))
                    kvpItemType[normalItem.ItemType] = 0;

                kvpItemType[normalItem.ItemType]++;

            }
        }

        if(kvpItemType.Count == 0) return FindFirstNonEmptyCell();

        List<NormalItem.eNormalType> eNormalTypes = new List<NormalItem.eNormalType>();
        foreach(var kvp in kvpItemType)
        {
            if(kvp.Value >= 2)
            {
                eNormalTypes.Add(kvp.Key);
            }
        }
        
        return FindCellWithNormalTypeExcept(eNormalTypes);
    }
    
    internal Cell FindCellWithItemType(NormalItem.eNormalType type)
    {
        foreach(Cell cell in m_cells)
        {
            if(cell.Item != null)
            {
                NormalItem normalItem = cell.Item as NormalItem;
                if(normalItem.ItemType == type) return cell;
            }
        }
        return null;
    }
    internal Cell FindCellToWin(List<Cell> cells)
    {
        ...
        foreach(var kvp in kvpItemType)
        {
            if(kvp.Value >= 2)
            {
                return FindCellWithItemType(kvp.Key);
            }
        }
        NormalItem.eNormalType anyType = kvpItemType.Keys.First();
        return FindCellWithItemType(anyType);
    }
    internal void FillForStateSelect()
    {
        Vector2Int[] pairIndices  = new Vector2Int[boardSizeX*boardSizeY];
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                pairIndices[x * boardSizeY + y] = new Vector2Int(x , y);
            }
        }



        for (int i = pairIndices.Length - 1; i > 0; i--)
        {
            int j = UnityEngine.Random.Range(0, i + 1);

            Vector2Int temp = pairIndices[i];
            pairIndices[i] = pairIndices[j];
            pairIndices[j] = temp;
        }

        for(int i = 0 ; i< pairIndices.Length ; i+= 3)
        {
            ...
            for(int j = 2 ; j >= 0 ; j--)
            {
                ...
                Cell cell = m_cells[pairIndices[i + j].x, pairIndices[i+j].y];
                item.SetIndexOrigin(new Vector2Int(pairIndices[i + j].x, pairIndices[i+j].y));
                cell.Assign(item);

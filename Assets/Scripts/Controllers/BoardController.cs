...
public class BoardController : MonoBehaviour
{
    ...
    private SelectBoard m_selectBoard;
    ...
    public void StartGame(GameManager gameManager, GameSettings gameSettings)
    {
        ...
        m_selectBoard= new SelectBoard(this.transform , gameSettings);
    ...
    internal bool SelectStateLose()
    {
        return m_selectBoard.IsCellsItemCountFull();
    }
    internal bool IsEmpty()
    {
        if(m_board.IsEmpty()) return true;
        return false;
    }
    private void Fill()
    {
        m_board.FillForStateSelect();
    ...
    public void Update()
    {
        ...
        //     }
        // }
        switch(m_gameManager.Mode)
        {
            case GameManager.eLevelMode.SELECTIONS:
            case GameManager.eLevelMode.TIMER:
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (hit.collider != null)
                    {
                        ...
                        if(c is CellSelect cellSelect)
                        {
                            if(m_gameManager.Mode == GameManager.eLevelMode.TIMER  && c.Item != null )
                            {
                                IsBusy =true;
                                Debug.Log("Timmer call IsBusy");
                                Vector2Int indexOriginItem = m_selectBoard.GetIndexCellRedo(cellSelect);

                                m_board.ReceiveRedo( indexOriginItem ,  cellSelect );

                                m_selectBoard.RemoveItemAtIndex(new Vector2Int(cellSelect.Index ,cellSelect.Index) , () =>
                                {
                                    OnMoveEvent();
                                    IsBusy = false;
                                    Debug.Log("yesssbaby");
                                ...
                                DOTween.Sequence().AppendInterval(0.5f).OnComplete(() => { IsBusy = false ;});
                            }
                        }
                        else
                        {
                            IsBusy =true;
                            m_selectBoard.AddSelect(c,() =>{
                ...
                break;
            case GameManager.eLevelMode.AUTO_WIN:
                Cell c1 = m_board.FindCellToWin(m_selectBoard.Cells );  
                if(c1 == null) return;  
                    IsBusy =true;
                    m_selectBoard.AddSelect(c1,() =>{
                        
                            StartCoroutine(IsBusyFreeAfterEndOfFrame());
                            OnMoveEvent();
                        });
                break;
            case GameManager.eLevelMode.AUTO_LOSE:
                Cell c2 = m_board.FindCellToLose(m_selectBoard.Cells);
                    if(c2 == null) return;
                    IsBusy =true;
                    m_selectBoard.AddSelect(c2,() =>{
                        Debug.Log("call");
                            StartCoroutine(IsBusyFreeAfterEndOfFrame());
                            OnMoveEvent();
                        });
    ...
    IEnumerator IsBusyFreeAfterEndOfFrame()
    {
